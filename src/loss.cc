...
Loss::Loss(std::shared_ptr<Matrix>& wo) : wo_(wo), paths_(), codes_() {
...
void Loss::findKBest(
  ...
  for (int32_t i = 0; i < output.size(); i++) {
    real output_val = output[i];
    if (output_val < threshold) {
      continue;
    }
    if (heap.size() == k && std_log(output_val) < heap.front().first) {
      continue;
    }
    heap.push_back(std::make_pair(std_log(output_val), i));
...
real BinaryLogisticLoss::binaryLogistic(
  ...
  if (backprop) {
    #if LARGE_BLOCKS
    wo_->doBackpropBinaryLogistic(state.grad, state.hidden, score, real(labelIsPositive), target, lr);
    #else
...
NegativeSamplingLoss::NegativeSamplingLoss(
  ...
  //std::minstd_rand r(time(0));
  //negctr = r() % negatives_.size();
}
...
real NegativeSamplingLoss::forward(
    ...
    real lr,
    bool backprop) {
  int32_t s = positives.size();
  real loss = 0;
  for (int32_t n = 0; n < s; n++) {
    if (positives[n] >= 0)
      loss = binaryLogistic(positives[n], state, true, lr, backprop);
  }
  for (int32_t n = 0; n < neg_; n++) {
    auto negativeTarget = negatives[n];
    loss += binaryLogistic(negativeTarget, state, false, lr, backprop);
  }
  return loss;
}
...
int32_t NegativeSamplingLoss::sharedNegatives(
    ...
    const std::vector<int32_t>& targets,
    std::minstd_rand& rng) {
  for (int32_t i = 0; i < negatives.size(); ++i) {
    int32_t negative;
    do {
      negative = negatives_[uniform_(rng)];
    } while (std::find(targets.begin(), targets.end(), negative) != targets.end());
...
real SoftmaxLoss::forward(
  ...
  if (backprop) {
    wo_->doBackpropSoftmax(state.grad, state.hidden, state.output, target, lr);
  }
