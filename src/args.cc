...
void Args::parseArgs(const std::vector<std::string>& args) {
  ...
  for (int ai = 2; ai < args.size(); ai += 2) {
    ...
    try {
      ...
      } else if (args[ai] == "-token-merges") {
        tokenMerges = std::string(args.at(ai + 1));
      } else if (args[ai] == "-token-vocab") {
        tokenVocab = std::string(args.at(ai + 1));
      } else if (args[ai] == "-mode") {
         if (args.at(ai + 1) == "normal") {
          mode = mode_name::normal;
        } else if (args.at(ai + 1) == "batched" && model == model_name::sg) {
          mode = mode_name::batched;
        } else if (args.at(ai + 1) == "ns") {
          mode = mode_name::ns;
        } else if (args.at(ai + 1) == "dh" && model == model_name::cbow) {
          mode = mode_name::uphid;
        } else if (args.at(ai + 1) == "dhf" && model == model_name::cbow) {
          mode = mode_name::uphid_fixed;
        } else if (args.at(ai + 1) == "dh-ns" && model == model_name::cbow) {
          mode = mode_name::uphid_ns;
        } else if (args.at(ai + 1) == "dhf-ns" && model == model_name::cbow) {
          mode = mode_name::uphid_ns_fixed;
        }else {
          std::cerr << "Unknown mode or not compatible with model: " << args.at(ai + 1) << std::endl;
          printHelp();
          exit(EXIT_FAILURE);
        }
      } else if (args[ai] == "-shared") {
        shared = std::stoi(args.at(ai + 1));
      } else if (args[ai] == "-no-subwords") {
        useSubwords = false;
        ai--;
      } else {
  ...
  if (tokenMerges != "" && tokenVocab == "" || tokenMerges == "" && tokenVocab != "") {
    std::cerr << "Both -token-vocab and -token-merges must be specified to run the tokenized version." << std::endl;
    printHelp();
    exit(EXIT_FAILURE);
  }
  if (!useSubwords && tokenMerges != "" && tokenVocab != "") {
    std::cerr << "Arguments -token-merges and -token-vocab are incompatible with -no-subwords." << std::endl;
    printHelp();
    exit(EXIT_FAILURE);
  }
  if (shared > 0 && mode != mode_name::ns && mode != mode_name::uphid_ns && mode != mode_name::uphid_ns_fixed) {
    std::cerr << "Argument -shared has no effect, please run with -mode {ns, dh-ns, dhf-ns}." << std::endl;
    printHelp();
    exit(EXIT_FAILURE);
  }
...
void Args::printTrainingHelp() {
  std::cerr
      ...
      << "\nFor the evaluation of Faster Training of Word Embeddings, use:\n"
      << "  -mode              mode of operation [normal]:\n"
      << "    normal           regular optimized (code_opt) fastText run\n"
      << "    batched          batched context (skipgram only)\n"
      << "    ns               enable negative sharing (sharing across context ns_ct for skipgram)\n"
      << "                                              for cbow, see argument -shared\n"
      << "    dh               update hidden layer dynamically (cbow only)\n"
      << "    dhf              update hidden layer dynamically, fixed contex window size (cbow only)\n"
      << "    dh-ns            dh with negative sharing (cbow only, see argument -shared)\n"
      << "    dhf-ns           dhf with negative sharing (cbow only, see argument -shared)\n"
      << "  -shared            number of consecutive words sharing negative samples [0]\n"
      << "                     (cbow only, -shared 0 results in sharing across 2 * ws + 1)\n"
      << "  -no-subwords       disables subwords (word2vec); incompatible with -token-merges and -token-vocab\n"
      << "  -token-merges      path to .txt file with BPE token merges []: must be set with -token-vocab\n"
      << "  -token-vocab       path to .json file with BPE token vocab []: must be set with -token-merges\n";
}
