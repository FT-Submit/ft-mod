...
void QuantMatrix::addVectorToRows(const Vector&, std::vector<int32_t>::const_iterator,
    std::vector<int32_t>::const_iterator, real) {
  throw std::runtime_error("Operation not permitted on quantized matrices.");
}

void QuantMatrix::addRowsToVector(Vector& x, std::vector<int32_t>::const_iterator start,
...
void QuantMatrix::dotRows(const Vector& vec, Vector& target) const {
  throw std::runtime_error("Operation not permitted on quantized matrices.");
}

void QuantMatrix::doBackpropSoftmax(Vector& grad, Vector& hidden, Vector& output, int32_t target, real lr) {
  throw std::runtime_error("Operation not permitted on quantized matrices.");
}

void QuantMatrix::doBackpropBinaryLogistic(Vector& grad, Vector& hidden, real score, real label, int32_t target, real lr) {
  throw std::runtime_error("Operation not permitted on quantized matrices.");
}
