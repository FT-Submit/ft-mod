...
struct IdCount {
  IdCount(int32_t i, int32_t c) : id(i), count(c) {}
  int32_t id;
  int32_t count;
};
...
class Dictionary {
 protected:
  ...
  // mapping utf-8 characters to huggingface tokenizer's byte map
  std::string* byteCharMap;
 ...
 public:
  ...
  std::map<std::string, int32_t> token_vocab;
  std::map<std::pair<int32_t, int32_t>, std::pair<int32_t, int32_t>> token_merges;
  void readFromFileTokenized(std::istream&, std::istream&, std::istream&);
  void readTokenVocab(std::istream& in);
  void readTokenMerges(std::istream& in);
  void initTokens();
  void computeTokens(const std::string& word, std::vector<int32_t>& ngrams);
