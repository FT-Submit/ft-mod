...
Vector::Vector(int64_t m) : l_(m), lvec_((m + 15) / 16 * 16) {
...
void Vector::mul(real a) {
  ...
  for (int64_t i = 0; i < lvec_; i += 16) {
    _mm512_storeu_ps(&data_[i], _mm512_mul_ps(_mm512_loadu_ps(&data_[i]), avec));
  }
...
void Vector::addVector(const Vector& source) {
  ...
  //int64_t vsize16 = vsize - (vsize & 15);
  for (int64_t i = 0; i < lvec_; i += 16) {
    _mm512_storeu_ps(&data_[i], _mm512_add_ps(_mm512_loadu_ps(&data_[i]),
        _mm512_loadu_ps(&source.data_[i])));
  }
  //for (int64_t i = vsize16; i < vsize; i++) {
...
void Vector::addVector(const Vector& source, real s) {
  ...
  __m512 svec = _mm512_set1_ps(s);
  for (int64_t i = 0; i < lvec_; i += 16) {
    _mm512_storeu_ps(&data_[i], _mm512_fmadd_ps(_mm512_loadu_ps(&source.data_[i]),
        svec, _mm512_loadu_ps(&data_[i])));
  }
  //for (int64_t i = vsize16; i < vsize; i++) {
