...
void Dictionary::initNgrams() {
  for (size_t i = 0; i < size_; i++) {
    ...
    if (args_->useSubwords && words_[i].word != EOS) {
...
void Dictionary::readTokenVocab(std::istream& in) {
  token_vocab.clear();
  ...
  while (getline(in, pair, ',')) {
    ...
    std::string key;
    int32_t value;
    int32_t colon = pair.rfind(':');
    key = pair.substr((i == 0 ? 2 : 1), (i == 0 ? colon - 3 : colon - 2));
    value = std::stoi(pair.substr(colon + 1, pair.length() - colon - 1));
    token_vocab.insert({key, value});
    ++i;
  }
}

void Dictionary::readTokenMerges(std::istream& in) {
  token_merges.clear();
  std::string line;
  int32_t r = 0;
  int32_t i = 0;
  ...
  while (getline(in, line)) {
    ...
    ss >> lword >> rword; 
    ++i;
    //if (token_vocab.count(lword) > 0 && token_vocab.count(rword) > 0 && token_vocab.count(lword + rword) > 0) {
      int32_t lid = token_vocab.at(lword);
      int32_t rid = token_vocab.at(rword);
      int32_t mid = token_vocab.at(lword + rword);
      token_merges.insert({std::pair<int32_t, int32_t>(lid, rid), std::pair<int32_t, int32_t>(mid, r++)});
    //}
  }
}

void Dictionary::computeTokens(
    const std::string& word,
    std::vector<int32_t>& ngrams) {
  std::vector<std::string> w;
  //"Ġ" in utf-8 takes 2 characters when represented as std::string
  w.push_back(word.substr(0, 2));
  //the remaining characters are mapped to huggingface's 0...255 character set
  for (int32_t i = 2; i < word.size(); ++i) w.push_back(byteCharMap[(int)(unsigned char)(word[i])]);
  int32_t size, minrank, minmid;
  std::string minlword, minrword;
  while (true) {
    size = w.size();
    minrank = INT32_MAX;
    for (int32_t i = 0; i < size - 1; ++i) {
      std::string lword = w[i];
      std::string rword = w[i + 1];
      if (token_vocab.count(lword) > 0 && token_vocab.count(rword) > 0) {
        std::pair<int32_t, int32_t> lidrid(token_vocab.at(lword), token_vocab.at(rword));
        if (token_merges.count(lidrid) > 0) {
          std::pair<int32_t, int32_t>& midrank = token_merges.at(lidrid);
          if (minrank > midrank.second) {
            minrank = midrank.second;
            minlword = lword;
            minrword = rword;
          }
        }
    ...
    if (minrank == INT32_MAX) break;
    //merge word
    std::string mword = minlword + minrword;
    for (int32_t i = 0; i < size - 1; ++i) {
      if (w[i] == minlword && w[i + 1] == minrword) {
        w[i] = mword;
        w.erase(w.begin() + i + 1);
        size = w.size();
      }
  ...
  //insert all created tokens as ngrams
  for (int32_t i = 0; i < w.size(); ++i) {
    int32_t h = hash(w[i]) % args_->bucket;
    pushHash(ngrams, h);
  }
}

void Dictionary::initTokens() {
  for (size_t i = 0; i < size_; i++) {
    std::string word = "Ġ" + words_[i].word;
    words_[i].subwords.clear();
    words_[i].subwords.push_back(i);
    if (word != EOS) {
      computeTokens(word, words_[i].subwords);
    }
  }
}

void Dictionary::readFromFileTokenized(std::istream& in, std::istream& vocab_in, std::istream& merges_in) {
  
  // a 0...255 character map used by huggingface tokenizer
  byteCharMap = new std::string[256] {
    ...
    "u", "v", "w", "x", "y", "z", "{", "|", "}", "~", "ġ", "Ģ", "ģ",
    "Ĥ", "ĥ", "Ħ", "ħ", "Ĩ", "ĩ", "Ī", "ī", "Ĭ", "ĭ", "Į", "į", "İ",
    "ı", "Ĳ", "ĳ", "Ĵ", "ĵ", "Ķ", "ķ", "ĸ", "Ĺ", "ĺ", "Ļ", "ļ", "Ľ",
    "ľ", "Ŀ", "ŀ", "Ł", "ł", "¡", "¢", "£", "¤", "¥", "¦", "§", "¨",
    "©", "ª", "«", "¬", "Ń", "®", "¯", "°", "±", "²", "³", "´", "µ",
    "¶", "·", "¸", "¹", "º", "»", "¼", "½", "¾", "¿", "À", "Á", "Â",
  ...
  readTokenVocab(vocab_in);
  readTokenMerges(merges_in);
